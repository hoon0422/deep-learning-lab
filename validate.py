...
parser.add_argument('--lab', default='dhan_fusion', type=str)  # , required=True)
parser.add_argument('--instance', default='dhan_fusion', type=str)  # , required=True)
parser.add_argument('--distributed', action='store_true', default=False)
parser.add_argument('--mp', type=str, required=False)
parser.add_argument('--master_port', type=int, required=False)
args = parser.parse_args()

param = import_module(f'laboratory.{args.lab}.{args.instance}.param_val')
args_to_pass = option.params_to_argv(param.get_params(args.lab, args.instance, args.distributed))

if not args.distributed:
    validate = import_module(f'laboratory.{args.lab}.{args.instance}.validate')
    validate.main(args_to_pass)
else:
    ...
    os.system(
        ...
        f'laboratory/{args.lab}/{args.instance}/validate.py {" ".join(args_to_pass)}'
    )
